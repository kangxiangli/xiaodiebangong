		...
		success: function(data) {
			...
			for(var i = 0; i < data.length; i++) {
				...
				for(var j = 0; j < admins.length; j++) {
					var admin = admins[j];
					var img = '<img class="tx_hd" src="' + imgSrc + admin.UserPhoto + '" alt="头像" />';
...
$(function() {
	$(".kq_tx").click(function(e) {
		...
		$("#right_box,#right_box_shade").css("transform", "translateZ(-100px)translateX(55%)rotateY(-45deg)").css("-webkit-transform", "translateZ(-100px)translateX(55%)rotateY(-45deg)").css("-moz-transform", "translateZ(-100px)translateX(55%)rotateY(-45deg)").css("-o-transform", "translateZ(-100px)translateX(55%)rotateY(-45deg)");
	...
	function animatend() {
		...
		$("#right_box").css("transform", "translateZ(0)translateX(0)rotateY(0)").css("-webkit-transform", "translateZ(0)translateX(0)rotateY(0)").css("-moz-transform", "translateZ(0)translateX(0)rotateY(0)").css("-o-transform", "translateZ(0)translateX(0)rotateY(0)").css("background-color", "rgba(134,119,91,0)");
		...
		$.xiaodie.ajax({
			...
			success: function(data) {
				...
				if(data.ResultType == 3) {
					...
					var resultinfo = "";
					var option = $('<li><label style="width:33%" class="_date"></label><b class="lazy" style="width:33%" ></b> <span class="_info "  onclick="repair(this)"></span></li>');
					$.each(entities, function(index, item) {
						var option = $('<li><label style="width:33%" class="_date"></label><b class="lazy" style="width:33%" ></b> <span class="_info "  onclick="repair(this)"></span></li>');
						var id = item.Id;
						$("#count").html(index);
						var AdminScore = item.AdminScore;
						var PaidScore = item.PaidScore;
						var Type = item.Type;
//						console.log("type"+JSON.stringify(Type))
						option.find("._date").html(item.AttendanceTime);
						option.find(".lazy").html(getCurTypeLazy(item.Type));
						option.find("._info").html(getCurTypeInfo(item.Type)).attr("data-id", item.Id).attr('data-type', item.Type).attr("data-AdminScore", item.AdminScore).attr("data-PaidScore", item.PaidScore);

						resultinfo += option[0].outerHTML;

						$(".end_card .info").append(resultinfo);
					})
				} else {
	...
	$("#inpstart").mobiscroll(optDateTime).datetime(optDateTime);
	//请假开始日期
	$("#inpend").mobiscroll(optDateTime).datetime(optDateTime);
	//请假结束日期
	$("#jbstart").mobiscroll(optDateTime).datetime(optDateTime);
	//加班开始日期
	$("#jbend").mobiscroll(optDateTime).datetime(optDateTime);
	//加班截止日期
	$("#wqstart").mobiscroll(optDateTime).datetime(optDateTime);
	//外勤开始日期
	$("#wqend").mobiscroll(optDateTime).datetime(optDateTime);
	//外勤截止日期
	$("#bktime").mobiscroll(opDate).date(opDate);
	//补卡日期

	$(".dialog_ul li").click(function() {
		var ind = $(this).index();
		//if (ind < 3) {
		$(".apply_form,.foot_apply").animate({
			left: '0'
		}).css("display", "block");
...
//			console.log(JSON.stringify(data.Data.Type))
			if(data.ResultType == 3) {
				...
				$.each(entities, function(index, item) {
				var option = $('<li class="wqt_box"><label class="_date"></label> <span id="wqc" class="_info " onclick="repair(this)"></span></li>');
//					var curitem = option.clone();
					var id = item.Id;
					var AdminScore = item.AdminScore;
					var PaidScore = item.PaidScore;
					var Type = item.Type;
//					console.log(JSON.stringify(Type))
					option.find("._date").html(item.AttendanceTime);
					// curitem.find(".lazy").html(item.Type);
					option.find("._info").html(getCurTypeInfo(item.Type)).attr("data-id", item.Id).attr('data-type', item.Type).attr("data-AdminScore", item.AdminScore).attr("data-PaidScore", item.PaidScore);
					//curitem.find(".lazy").html(getCurTypeLazy(item.Type));
					resultinfo += option[0].outerHTML;
					$(".sign_ul").append(resultinfo);
...
//				console.log(JSON.stringify(entities))
				...
				var option = $('<li><label class="_date"></label><b class="lazy"></b> <span class="_info cdbk" onclick="repair(this)"></span></li>');
				$.each(entities, function(index, item) {
					var option = $('<li class="cdbk_box"><label class="_date"></label><b class="lazy"></b> <span class="_info cdbk " onclick="repair(this)"></span></li>');
//					var curitem = option.clone();
					var id = item.Id;
					...
					option .find("._date").html(item.AttendanceTime);
					// curitem.find(".lazy").html(item.Type);
					option .find(".cdbk").html(getCurTypeInfo(item.Type)).attr("data-id", item.Id).attr('data-type', item.Type).attr("data-AdminScore", item.AdminScore).attr("data-PaidScore", item.PaidScore);
					option .find(".lazy").html(Minutes);
					resultinfo += option[0].outerHTML;
					$(".bk_ul").append(resultinfo);
				})
	...
	$.xiaodie.ajax({
		...
		success: function(data) {
			if(data.ResultType == 3) {
				...
				var resultinfo = "";
				var option = $('<li><label class="_date"></label><span class="_info "></span></li>');
				$.each(entities, function(index, item) {
					var curitem = option.clone();
					curitem.find("._date").html(item.AttendanceTime);
					//curitem.find("._info").html(getCurTypeInfo(item.Type)).prop("data-id",item.Id);
					curitem.find("._info").html(getCurTypeInfo(item.Type));

					resultinfo += curitem.html();
				});
				$(".end_card .info").append(resultinfo);
			} else {
				showMsg(data.Message);
...
function getCurTypeLazy(attenFlag) {
...
//判断Type类型
function getCurTypeInfo(attenFlag) {
	var info = "";
	switch(attenFlag) {
		case 0:
			info = "审核中";
...
function repair(option) {
	...
	var PaidScore = $(option).attr("data-PaidScore");
	var AdminScore = $(option).attr("data-AdminScore");
	if(type == 3) {
		var msg = "当前总积分为" + AdminScore + "分,应扣除" + PaidScore + "分"
		showDialog(msg, id, getConfirmRepair(id, AdminScore, PaidScore))
			//扣积分
		return;
	}
	var adminId = localStorage.AdminId
	$.xiaodie.ajax({
		url: "/Atten/ApplyRepair",
		type: "POST",
		data: {
			Id: id,
			AdminId: adminId,
...
//type类型为3时  点击待确认执行的函数
function getConfirmRepair(id, AdminScore, PaidScore, AttenFlag) {
	var adminId = localStorage.AdminId;
	$.xiaodie.ajax({
		url: "/Atten/ConfirmRepair",
		type: "POST",
		data: {
			Id: id,
			AdminId: adminId,
			AttenFlag: attenFlag,
			AdminScore: AdminScore, //总积分
			PaidScore: PaidScore //应扣除积分
